...
/**
 * Bird snaring task implementation for hunting birds with traps.
 * This is a specific implementation that can be extended for other trap types.
 */
public class BirdSnaringTask extends AbstractHuntingTask {
    ...
    public BirdSnaringTask(JorkHunter script, TrapType trapType, int maxTraps, List<RectangleArea> huntingZones) {
        this(script, trapType, maxTraps, huntingZones, new NoCardinalStrategy());
    }

    public BirdSnaringTask(JorkHunter script, TrapType trapType, int maxTraps, List<RectangleArea> huntingZones, TrapPlacementStrategy placementStrategy) {
        super(script, trapType, maxTraps, huntingZones, placementStrategy);
    }
    
    @Override
    protected String getTaskName() {
        return "BirdSnaringTask";
    }
    ...
    @Override
    public int execute() {
        // CRITICAL: Check if we're still laying a trap
        // Detect completion by checking if player moved exactly 1 tile from start position
        if (trapManager.isCurrentlyLayingTrap()) {
            WorldPosition currentPos = script.getWorldPosition();
            
            // Check for timeout (more than 5 seconds since starting)
            long timeSinceStart = System.currentTimeMillis() - trapLayingStartTime;
            if (trapLayingStartTime > 0 && timeSinceStart > 5000) {
                ScriptLogger.warning(script, "Trap laying timed out after " + timeSinceStart + "ms - aborting");
                
                // Clear the laying state
                WorldPosition trapPos = trapManager.getCurrentlyLayingPosition();
                if (trapPos != null) {
                    trapManager.completeTrapLaying(trapPos, false);
                } else {
                ...
                // Return and let the script continue
                return Utils.random(500, 800);
            }
            
            if (trapLayingStartPosition != null && currentPos != null) {
                double distance = currentPos.distanceTo(trapLayingStartPosition);
                
                // Check if we've moved exactly 1 tile (any direction including diagonal)
                // This indicates the laying animation has completed
                if (distance >= 1.0 && distance <= 1.5) {  // 1.0 for cardinal, ~1.41 for diagonal
                    ScriptLogger.debug(script, "Trap laying complete - player moved from " + 
                                     trapLayingStartPosition + " to " + currentPos + " (distance: " + distance + ")");
                    
                    // Complete the trap laying process (transitions to ACTIVE state)
                    // We need to get the position from the manager since we're tracking it there
                    WorldPosition trapPos = trapManager.getCurrentlyLayingPosition();
                    if (trapPos != null) {
                        trapManager.completeTrapLaying(trapPos, true);
                    } else {
                        ScriptLogger.warning(script, "Could not get trap position for completion");
                        trapManager.clearLayingFlag();
                    }
                    
                    // Safe to scan now that we're away from trap position
                    trapManager.scanAndUpdateTrapStates();
                    
                    // Add a small randomized delay before the human task to let animation complete
                    script.sleep(Utils.random(25, 400));
                    
                    // Add humanized delay after trap laying completion
                    script.submitHumanTask(() -> true, Utils.random(400, 800));
                    
                    // Reset start position and time
                    trapLayingStartPosition = null;
                    trapLayingStartTime = 0;
                    
                    // Return longer delay to let the human delay complete
                    return Utils.random(600, 1000);
                } else if (distance > 1.5) {
                    // Moved too far - something went wrong
                    ScriptLogger.warning(script, "Player moved too far during trap laying (distance: " + 
                                       distance + ") - resetting");
                    WorldPosition trapPos = trapManager.getCurrentlyLayingPosition();
                    if (trapPos != null) {
                        trapManager.completeTrapLaying(trapPos, false);
                    ...
                    ScriptLogger.debug(script, "Still laying trap - waiting for movement (distance: " + distance + ", time: " + timeSinceStart + "ms)");
                    // Don't scan while still at/near start position
                    return Utils.random(200, 300); // Poll frequently to detect movement
                }
            } else {
                // Safety: if we can't get positions, clear the flag
                ScriptLogger.warning(script, "Cannot determine positions during trap laying - clearing flag");
                trapManager.clearLayingFlag();
                trapLayingStartPosition = null;
                trapLayingStartTime = 0;
            }
        ...
        if (!skipTrapHandling && priorityTrap.isPresent()) {
            ...
            if (trapToHandle != null) {
                ...
                if (isFreshFinished && !script.isDrainingForBreak() && !trapManager.isCurrentlyLayingTrap() 
                    ...
                    // Use a random threshold between 20-40 seconds for human-like behavior
                    long freshnessThreshold = Utils.random(20000, 40000);
                    
                    // Only skip if not urgent (collapsed traps become urgent after 60 seconds)
            ...
            case FAILED -> {
                ...
                if (verifyResult.success()) {
                    ...
                    if (inventoryChanged) {
                        ...
                        ScriptLogger.warning(script, "Blind tap succeeded but inventory unchanged - removing for re-scan");
                        trapManager.removeTrap(trapPos);
                    }
                } else {
                    // Remove problematic trap from tracking after multiple failures
                    trapManager.removeTrap(trapPos);
                }
            }
